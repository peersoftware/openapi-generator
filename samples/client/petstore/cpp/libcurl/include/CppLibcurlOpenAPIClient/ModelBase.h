...
public:
    ...
    template<typename T>
    static bool fromJson(const nlohmann::json& val, std::map<std::string, T> &);
...
template<typename T>
nlohmann::json ModelBase::toJson(const std::map<std::string, T>& val) {
...
bool ModelBase::fromJson(const nlohmann::json& val, std::vector<T> &outVal) {
    ...
    if (val.is_array()) {
        for (auto jitem = val.begin(); jitem != val.end(); ++jitem) {
            T item;
...
template<typename T>
bool ModelBase::fromJson(const nlohmann::json& val, std::map<std::string, T> &outVal) {
    bool ok = true;
    ...
    if (val.is_object()) {
        for (auto jitem = val.begin(); jitem != val.end(); ++jitem) {
            ...
            outVal.insert(std::pair<std::string, T>(jitem.key(), itemVal));
        }
    } else {
        ok = false;
    }
