...
template<typename T>
nlohmann::json ModelBase::toJson(const std::set<T>& value) {
    auto ret = nlohmann::json::array();
...
template<typename T>
bool ModelBase::fromJson(const nlohmann::json& val, std::set<T> &outVal) {
    bool ok = true;
...
bool ModelBase::fromJson(const nlohmann::json& val, std::map<std::string, T, std::less<>> &outVal) {
    ...
    if (val.is_object()) {
        for (const auto &[jkey, jvalue] : val.items()) {
            T itemVal;
            ok &= fromJson(jvalue, itemVal);
            outVal.try_emplace(jkey, std::move(itemVal));
        }
    } else {
        ok = false;
    }
